...
with open(valuespath, "r") as f:
    ...
    for key, values in envs.items():
        ...
        if "keyvault" in value:
            matches = re.search('\${keyvault:(?P<keyvault>.+)/(?P<secret>.+)\}', value)
            kv = matches.group('keyvault')
            secret = matches.group('secret')
            p = subprocess.Popen('az keyvault secret show --vault-name %s --name %s | jq .value' %(kv, secret), 
                    shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = [entry.decode('utf-8').strip() for entry in p.communicate()]
            if err:
                if debug:
